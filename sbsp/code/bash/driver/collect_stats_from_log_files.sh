...
awk -F "," '{if (NR > 1) print}' $pf_genome_list | while read -r line; do
    ...
    ancestor=$(echo "$line" | sed -E 's/.+ancestor=([^;]+).*/\1/g')

    quick_filters=$(grep "Quick filter"  $exp/v1_*${gcfid}*/steps/pbs_logs/* | sed -E 's/.+: ([0-9]+) -> ([0-9]+).*$/\1,\2/g')

    echo "$quick_filters" | awk -v gcf=$gcfid -v anc=$ancestor '{print gcf","anc","$0}' >> $pf_output
