...
This tool requires significant computational power and time. You can reduce this by tuning the parallelization parameters found in =$config/parallelization_defaults.conf=.

The setup is still in beta version. The tool is currently limited to a particular directory structure, with a fixed type of input (i.e. full genome, with a specific database construction). This was done to facilitate development and reduce potential for errors. I am currently working on refactoring and allowing more diverse set of input, and relaxing the constraints on database structure so that users can use a BLASTp or Diamond BLASTp database that they already have. This is still very much work in progress.
...
#+begin_src bash
conda env create -f install/biogem_sbsp.yml
conda activate biogem_sbsp
#+end_src

The first command need only be executed once to install packages. The second command has to be run every time a new shell is opened.
